...
$s = mysql_query ("SELECT * FROM `psbcalon_useraura` WHERE `UserId`='$id'");	
$data = mysql_fetch_array($s);
$user = $data['user'];	
$hapus = mysql_query ("DELETE FROM `psbcalon_useraura` WHERE `UserId`='$id' AND `user`!='admin'");	
if ($hapus){
...
if ($koneksi_db->sql_numrows($koneksi_db->sql_query("SELECT email FROM psbcalon_useraura WHERE email='$email' and UserId!='$id'")) > 0) $error .= "Error: Email ".$email." sudah terdaftar , silahkan ulangi.<br />";
if ($error) {
$admin.='<div class="error">'.$error.'</div>';
} else {
$up = mysql_query ("UPDATE `psbcalon_useraura` SET `level`='$level',`tipe`='$tipe',`email`='$email' WHERE `UserId`='$id' AND `user`!='admin'");	
...
if ($koneksi_db->sql_numrows($koneksi_db->sql_query("SELECT user FROM psbcalon_useraura WHERE user='$user'")) > 0) $error .= "Error: Username ".$user." sudah terdaftar , silahkan ulangi.<br />";
if ($koneksi_db->sql_numrows($koneksi_db->sql_query("SELECT email FROM psbcalon_useraura WHERE email='$email'")) > 0) $error .= "Error: Email ".$email." sudah terdaftar , silahkan ulangi.<br />";
if (!is_valid_email($email)) $error .= "Error: E-Mail address invalid!<br />";
if ($error){
        $admin.='<div class="error">'.$error.'</div>';
}else{
$query = mysql_query ("INSERT INTO `psbcalon_useraura` (`user`,`password`,`level`,`tipe`,`email`) VALUES ('$user',md5('$password'),'$level','$tipe','$email')");	
$admin .= '<div class="sukses">Data : '.$user.',Berhasil Di add</div>';
}
}	

$ss = mysql_query ("SHOW FIELDS FROM psbcalon_useraura");
...
$koneksi_db->sql_query("DELETE FROM psbcalon_useraura WHERE UserId in($pcheck)");
if ($sukses){
...
if($search){
$query = $koneksi_db->sql_query("SELECT count(user) as t FROM `psbcalon_useraura` WHERE user like '%$search%' or email like '%$search%'");
}else{
$query = $koneksi_db->sql_query("SELECT count(user) as t FROM `psbcalon_useraura` where user<>'superadmin'");
}
...
if ($jumlah > 0){
if($search){
$q = mysql_query ("SELECT * FROM `psbcalon_useraura` WHERE user like '%$search%' or email like '%$search%' LIMIT $offset,$limit");
}else{
$q = mysql_query ("SELECT * FROM `psbcalon_useraura` where user<>'superadmin' LIMIT $offset,$limit");
}
if($offset){
$no = $offset+1;
}else{
