...
$hasil = $koneksi_db->sql_query( "SELECT password,email FROM psbcalon_useraura WHERE user='$user'" );
...
if ($koneksi_db->sql_numrows($koneksi_db->sql_query("SELECT email FROM psbcalon_useraura WHERE email='$email' and user!='$user'")) > 0) $error .= "Error: Email ".$email." sudah terdaftar , silahkan ulangi.<br />";
...
$hasil = $koneksi_db->sql_query( "UPDATE psbcalon_useraura SET user='$user', email='$email', password='$password3' WHERE user='$user'" );
...
$hasil =  $koneksi_db->sql_query( "SELECT * FROM psbcalon_useraura WHERE user='$user'" );
while ($data = $koneksi_db->sql_fetchrow($hasil)) {
$id		= $data[0];
$user	= $data[1];
$email	= $data[3];
...
unlink($uploaddir);
$hasil = $koneksi_db->sql_query( "UPDATE psbcalon_useraura SET nama='$nama', alamat='$alamat', telepon='$telepon', web='$web', ym='$ym', avatar='$namagambar' WHERE UserId='$id'" );

$admin.='<div class="alert alert-success">Profil telah di updated</div>';
}else{
$hasil = $koneksi_db->sql_query( "UPDATE psbcalon_useraura SET nama='$nama', alamat='$alamat', telepon='$telepon', web='$web', ym='$ym' WHERE UserId='$id'" );
...
$hasil =  $koneksi_db->sql_query( "SELECT * FROM psbcalon_useraura WHERE user='$username'" );
while ($data = $koneksi_db->sql_fetchrow($hasil)) {
$id			= $data['id'];
$user		= $data['user'];
$nama		= $data['nama'];
...
$password3=md5($password1);
$hasil = $koneksi_db->sql_query( "UPDATE psbcalon_situs SET publishwebsite='$publishwebsite', judul_situs='$judul_situs', url_situs='$url_situs', slogan='$slogan', email_master='$email_master', description='$description', keywords='$keywords', alamatkantor='$alamatkantor' WHERE id='1'" );

$admin.='<div class="alert alert-success">Informasi Situs telah di updated</div>';
}
}
$user =  $_SESSION['UserName'];
$hasil =  $koneksi_db->sql_query( "SELECT * FROM psbcalon_situs WHERE id='1'" );
while ($data = $koneksi_db->sql_fetchrow($hasil)) {
$id				= $data['id'];
