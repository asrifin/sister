...
$admin.='<div class="error">'.$error.'</div>';
	...
	if($password<>''){
$up = mysql_query ("UPDATE `useraura` SET `level`='$level',`tipe`='$tipe',`email`='$email',`password`='$password' WHERE `UserId`='$id'");	
	}else{
$up = mysql_query ("UPDATE `useraura` SET `level`='$level',`tipe`='$tipe',`email`='$email' WHERE `UserId`='$id'");			
	}
...
';
$admin.='<tbody>';
$q = mysql_query ("SELECT * FROM `useraura` where user<>'superadmin'");
