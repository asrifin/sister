...
if($kodecustomer=='Semua'){
         ...
         $wherekodecustomer="and kodecustomer='$kodecustomer'";
}
switch ($carabayar) {
   case 'Semua':
         $wherestatus="";
   ...
   case 'Tunai':
         ...
         break;
   case 'Pemesanan':
         $wherestatus="and carabayar='Pemesanan'";
         break;
}
echo "<html><head><title>Laporan Pemesanan </title>";
...
if($detail!='ok'){
echo'<tr><td colspan="7"><h4>Laporan Pemesanan, Dari '.tanggalindo($tglmulai).', Sampai '.tanggalindo($tglakhir).', Customer '.$namacustomer.'</h4></td></tr>';
echo '
<tr class="border">
<td>No</td>
<td>No.PO</td>
...
$no =1;
$s = mysql_query ("SELECT * FROM `pos_popenjualan` where tgl >= '$tglmulai' and tgl <= '$tglakhir' order by tgl asc");	
...
$user = $datas['user'];
$urutan = $no + 1;
$lihatslippo = '<a href="cetak_notapopenjualan.php?kode='.$datas['nopo'].'&lihat=ok"target="new">'.$datas['nopo'].'</a>';
echo '
<tr class="border">
...
echo'<tr><td colspan="8"><h4>Laporan Pemesanan, Dari '.tanggalindo($tglmulai).', Sampai '.tanggalindo($tglakhir).', Customer '.$namacustomer.', , Jenis / Barang : '.$namajenisproduk.'/'.$namakodebarang.'</h4></td></tr>';
echo '
<tr class="border">
<td>No</td>
<td>No.PO</td>
...
$no =1;
$s = mysql_query ("SELECT * FROM `pos_popenjualan` where tgl >= '$tglmulai' and tgl <= '$tglakhir' $wherestatus  $wheresupplier order by tgl asc");	
while($datas = mysql_fetch_array($s)){
$id = $datas['id'];
$nopo = $datas['nopo'];
$tgl = $datas['tgl'];
...
$lihatslippo = '<a href="cetak_notapopenjualan.php?kode='.$datas['nopo'].'&lihat=ok"target="new">'.$datas['nopo'].'</a>';
$urutan = $no + 1;
$s2 = mysql_query ("SELECT * FROM `pos_popenjualandetail` where nopo = '$nopo' $wherekodebarang order by id asc");	
