        ...
        settingFR +='<div style="overflow:scroll;height:600px;">'
                    +'<form autocomplete="off" onsubmit="idSV();return false;">' 
                        +'<input id="id_formH" type="hidden">' 
                            +'<table width="800px" class="table hovered bordered striped">'
                                +'<thead>'
                                    +'<tr style="color:white;"class="info">'
                                        +'<th width="20"class="text-left">Pilihan</th>'
...
/* form Cetak Label (add & edit) */
        function infoFR(id){
            ...
            if(id!=''){ // edit mode
                ...
                $.ajax({
                    url:dir,
                    data:'aksi=ambiledit&subaksi=info&replid='+id,
                    type:'post',
                    dataType:'json',
                    success:function(dt){
                        $('#barkode_formH').val(id);
                        $('#judulTB').val(dt.nilai);
                    }
                });        
    ...
    function loadView(id,kunci){
        if(kunci!=''){ //edit
            ...
            view('alamat').done(function(dt){
                $('#deskripsiTB').val(dt.row);
            });
        }
    }
